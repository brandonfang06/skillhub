...
| `SKILLHUB_GLOBAL_NAMESPACE_AUTO_JOIN_ENABLED` | ConfigMap | `false` | Set `true` only to add newly created local/OAuth accounts to the built-in `global` namespace. Existing membership is not changed. |
