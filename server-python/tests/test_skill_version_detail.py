...
def test_skill_version_detail_v1_route_returns_envelope() -> None:
    app = create_app()
    app.state.skill_version_detail_reader = lambda namespace, slug, version: detail_response(version)
...
def test_skill_version_detail_route_forwards_version_to_reader() -> None:
    seen: list[tuple[str, str, str]] = []
    app = create_app()

    def reader(namespace: str, slug: str, version: str) -> dict[str, object]:
        seen.append((namespace, slug, version))
        return detail_response(version)

    app.state.skill_version_detail_reader = reader

    client = TestClient(app)
    response = client.get("/api/v1/skills/global/demo/versions/1.0.0")

    assert response.status_code == 200
    assert seen == [("global", "demo", "1.0.0")]
