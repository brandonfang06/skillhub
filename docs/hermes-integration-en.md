...
| Component | Referenced version | Notes |
|-----------|-------------------|-------|
| SkillHub Server | `v0.2.14` | Public or self-hosted registry |
| SkillHub CLI | `0.1.9` | npm package `@astron-team/skillhub` |
| Hermes Agent | `0.18.2` | Upstream tag [`v2026.7.7.2`](https://github.com/NousResearch/hermes-agent/tree/v2026.7.7.2) |

Contract review date: 2026-07-26.
...
SkillHub CLI 0.1.9 overwrites a local skill by repeating the install command with `--force`. Omitting `--version` resolves the latest published version; you can also pin one explicitly:
...
Hermes lists and loads external skills alongside local skills. If the same skill name exists in the local directory and an `external_dirs` entry, Hermes 0.18.2 gives the local version precedence and shadows the external copy.
