...
| 组件 | 参考版本 | 说明 |
|------|------------|------|
| SkillHub Server | `v0.2.14` | 公开或自托管 registry |
| SkillHub CLI | `0.1.9` | npm 包 `@astron-team/skillhub` |
...
SkillHub CLI 0.1.9 使用同一安装命令加 `--force` 覆盖本地技能。省略 `--version` 会解析最新已发布版本；也可以显式固定版本：
...
Hermes 会把 external skill 与本地技能一起列出和加载。如果本地技能目录与 `external_dirs` 中存在同名技能，Hermes 0.18.2 会优先使用本地版本，并遮蔽 external 副本。
