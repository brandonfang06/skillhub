...
## One-Command Publish Storage Foundation Verification Gate

For the publish local storage write foundation milestone, use:
...
The command writes the latest storage foundation ownership summary to:

```text
.dev/publish-storage-foundation-contract-result.json
```

This gate verifies the publish storage foundation boundary:

- Python storage helper tests pass for object keys, local file writes, SHA-256 metadata, bundle zip
  bytes, and path safety checks.
- `POST /api/v1/skills` remains Java-owned through Vite.
- `POST /api/v1/publish` remains Java-owned through Vite.
- `POST /api/v1/skills/{namespace}/publish` remains Java-owned through Vite.
- `POST /api/web/skills/{namespace}/publish` remains Java-owned through Vite.
...
## One-Command Publish Side-Effect Foundation Verification Gate

For the publish side-effect foundation milestone, use:
...
The command writes the latest side-effect foundation ownership summary to:

```text
.dev/publish-side-effects-foundation-contract-result.json
```

This gate verifies the publish side-effect foundation boundary:

- Python side-effect helper tests pass for review task decisions, review/published event intents,
  scanner audit seed rows, scan task payload shape, non-published `SCANNING` transition, and
  ClawHub `COMPAT_PUBLISH` audit payload.
- `POST /api/v1/skills` remains Java-owned through Vite.
- `POST /api/v1/publish` remains Java-owned through Vite.
- `POST /api/v1/skills/{namespace}/publish` remains Java-owned through Vite.
- `POST /api/web/skills/{namespace}/publish` remains Java-owned through Vite.
- No Python publish HTTP route is called by this gate.
- The live gate does not call the scanner, publish to Redis streams, deliver notifications, or
  mutate the database through a Python route.
...
## One-Command Publish Orchestration Foundation Verification Gate

For the publish orchestration foundation milestone, use:
...
The command writes the latest orchestration foundation ownership summary to:

```text
.dev/publish-orchestration-foundation-contract-result.json
```

This gate verifies the publish orchestration foundation boundary:

- Python orchestration tests pass for prepare/storage/finalize/side-effect sequencing and
  after-commit replacement storage deletion.
- `POST /api/v1/skills` remains Java-owned through Vite.
- `POST /api/v1/publish` remains Java-owned through Vite.
- `POST /api/v1/skills/{namespace}/publish` remains Java-owned through Vite.
- `POST /api/web/skills/{namespace}/publish` remains Java-owned through Vite.
- No Python publish HTTP route is called by this gate.
- The live gate does not mutate the database through a Python route; HTTP request parsing, scanner
  delivery, CSRF/session behavior, and route ownership remain future milestone work.
