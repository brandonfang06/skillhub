...
| 116.2 | Auth helper import boundary and social route ordering | python | Remaining Python API route modules now import `read_current_mock_user` from `app.auth.context` instead of `app.api.auth`, keeping auth helper ownership out of the API layer. Social router registration now precedes broad lifecycle hard-delete routes so `DELETE /api/web/skills/{skillId}/star` and subscription routes reach the intended social handlers. |
| 116.3 | Shared platform role policy helpers | python | `app.auth.policy` now owns platform role normalization and reusable platform-role guards while preserving existing route-specific 403 detail strings. Admin search, label definitions, admin skill governance, admin users, audit logs, admin review/report routes, labels, lifecycle hard delete, namespaces, publish, security audit, and social list routes now use the shared role extraction/guard helpers. Superseded by 116.4 for namespace-role policy helpers. |
| 116.4 | Shared namespace role policy helpers | python | `app.auth.policy` now owns namespace role normalization, owner/manager/member predicates, generic namespace role allowance checks, and managed namespace ID selection. Namespace members/mutations/read models, skill lifecycle, promotion workflow, review approval/query, governance workbench, security audit, skill label checks, and skill read/detail visibility now use the shared namespace-role helpers. Hybrid lifecycle gates were updated to assert rerelease is Python-owned instead of stale Java-owned boundary wording. |
...
| 121 | Final route-policy hardening | python | Remaining protected API route modules now resolve mock/session principals through `app.auth.context.resolve_current_user_or_401` instead of route-local mock-only helpers. Static guardrails cover account merge, admin, governance, profile, device auth, report, security audit, labels, notifications, social, lifecycle, promotion, and review routes; the shared resolver owns unit-test mock fallbacks for route tests without a real DB engine. This was executed after milestone 120 and closes the final route-policy acceptance gap originally tracked under milestone 116. |
...
- Completed: shared principal and route-policy foundation:
  ...
  centralizes initial API-token scope plus unsupported-admin-route checks. Milestone 121 also
  removed route-local mock-only principal helpers from the remaining protected route modules,
  closing the final route-policy gap that originated in milestone 116.
- Still deferred outside the completed lifecycle/governance, schema ownership, route-policy, and
  runtime cutover audits: none currently tracked.
