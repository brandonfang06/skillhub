...
| GET | `/api/v1/user/profile` | python | Current user profile read moved to Python. Preserves Java current values, latest PENDING/REJECTED change request projection, PENDING self-view overlay, and field policy response. |
| PATCH | `/api/v1/user/profile` | python | Current user display-name update moved to Python. Preserves Java validation, default human-review queueing, immediate-apply mode, profile change request writes, and `PROFILE_UPDATE` audit for immediate apply. Spring session refresh remains deferred to final session replacement. |
| GET | `/api/v1/skills/{skillId}/versions/{versionId}/security-audit` | python | Authenticated security audit read moved to Python. Preserves Java version/skill mismatch errors, visibility checks, latest active audit selection per scanner type, optional `scannerType`, empty-list missing audit response, and Java response envelope. |
...
| POST | `/api/v1/tokens` | python | API token create/rotate moved to Python. Requires current user; bearer `api_token` principals must include `token:manage`; mock-user development auth keeps precedence. Trims and validates names, rotates active same-name tokens, stores SHA-256 hashes only, and returns raw `sk_` token once. |
...
| GET | `/api/web/skills/{namespace}/{slug}/versions` | python | Frontend alias for public published version list with manager-only owner preview lifecycle versions. Files metadata, compare, file bytes, and downloads are Python-owned. |
...
| GET | `/api/v1/skills/{namespace}/{slug}/versions/{version}` | python | Public published version detail with manager-only non-published owner preview access. Files metadata, compare, file bytes, and downloads are Python-owned; DELETE remains Java-owned. |
| GET | `/api/web/skills/{namespace}/{slug}/versions/{version}` | python | Frontend alias for public published version detail with manager-only non-published owner preview access. Files metadata, compare, file bytes, and downloads are Python-owned. |
...
| POST | `/api/v1/skills/{namespace}/publish` | python | Portal publish upload moved to Python and reuses the Python publish write service. Bearer `api_token` principals must include `skill:publish`; mock-user development auth keeps precedence. |
| POST | `/api/web/skills/{namespace}/publish` | python | Frontend publish upload alias moved to Python and reuses the Python publish write service. Bearer `api_token` principals must include `skill:publish`; mock-user development auth keeps precedence. |
| POST | `/api/cli/v1/skills/{namespace}/publish/validate` | python | CLI publish validate-only dry-run route. Bearer `api_token` principals must include `skill:publish`; mock-user development auth keeps precedence. Multipart adapter over Python dry-run model; no DB/storage publish writes. |
| POST | `/api/cli/v1/skills/{namespace}/publish` | python | CLI publish write moved to Python after publish foundation, replacement, pending-review auto-withdraw, scanner handoff, and rollback live gates. Bearer `api_token` principals must include `skill:publish`; mock-user development auth keeps precedence. |
...
| POST | `/api/v1/admin/skills/{skillId}/hide` | python | Platform-admin skill hide moved to Python. `SUPER_ADMIN` only; rejects bearer API-token principals as Java-compatible unsupported admin-route access, sets the hidden overlay, preserves `skill.status`, updates audit fields, and writes `HIDE_SKILL` audit with optional reason detail. |
| POST | `/api/v1/admin/skills/{skillId}/unhide` | python | Platform-admin skill unhide moved to Python. `SUPER_ADMIN` only; rejects bearer API-token principals as Java-compatible unsupported admin-route access, clears the hidden overlay, preserves `skill.status`, updates audit fields, and writes `UNHIDE_SKILL` audit. |
| POST | `/api/v1/admin/skills/versions/{versionId}/yank` | python | Admin version yank moved to Python. `SKILL_ADMIN` or `SUPER_ADMIN`; rejects bearer API-token principals as Java-compatible unsupported admin-route access, can yank only `PUBLISHED` versions, recalculate the skill latest pointer when needed, disable downloads, and write `YANK_SKILL_VERSION` audit. |
| GET | `/api/v1/admin/skills/{skillId}/resource-diagnostics` | python | Local Python maintenance extension. Session or mock `SUPER_ADMIN` only; rejects bearer API-token principals, reports database/file projection gaps, and performs bounded object-storage existence probes without affecting normal search traffic. |
...
| GET | `/api/v1/admin/labels` | python | Admin label definition list moved to Python. Requires `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves Java sort order and response shape. |
| POST | `/api/v1/admin/labels` | python | Admin label definition create moved to Python. Requires `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, normalizes slug/translations, writes label rows, and records `LABEL_CREATE` audit. |
| PUT | `/api/v1/admin/labels/{slug}` | python | Admin label definition update moved to Python. Requires `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, replaces translations, updates type/visibility/sort order, and records `LABEL_UPDATE` audit. |
| DELETE | `/api/v1/admin/labels/{slug}` | python | Admin label definition delete moved to Python. Requires `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, deletes the definition through DB cascade and records `LABEL_DELETE` audit. |
| PUT | `/api/v1/admin/labels/sort-order` | python | Admin label definition sort update moved to Python. Requires `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, updates per-label sort order and records `LABEL_SORT_ORDER_UPDATE` audit. |
| GET | `/api/v1/admin/users` | python | Admin user list moved to Python. Requires `USER_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves Java search/status/page behavior, created-at descending order, role fallback to `USER`, and page envelope. |
| PUT | `/api/v1/admin/users/{userId}/role` | python | Admin user role update moved to Python. Requires `USER_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, deletes previous platform bindings, preserves `USER` as no binding, and keeps `SUPER_ADMIN` assignment restricted to super admins. |
| PUT | `/api/v1/admin/users/{userId}/status` | python | Admin user status update moved to Python. Requires `USER_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, allows only Java-manageable `ACTIVE`/`DISABLED` transitions, and returns Java-compatible mutation envelope. |
| POST | `/api/v1/admin/users/{userId}/approve` | python | Admin user approve alias moved to Python. Rejects bearer API-token principals as Java-compatible unsupported admin-route access and sets status to `ACTIVE` with the same mutation contract as Java. |
| POST | `/api/v1/admin/users/{userId}/disable` | python | Admin user disable alias moved to Python. Rejects bearer API-token principals as Java-compatible unsupported admin-route access and sets status to `DISABLED` with the same mutation contract as Java. |
| POST | `/api/v1/admin/users/{userId}/enable` | python | Admin user enable alias moved to Python. Rejects bearer API-token principals as Java-compatible unsupported admin-route access and sets status to `ACTIVE` with the same mutation contract as Java. |
| POST | `/api/v1/admin/users/{userId}/password-reset` | python | Admin-triggered password reset moved to Python. Requires `USER_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves Java eligibility checks, consumes old pending requests, writes BCrypt-compatible admin reset rows, and exposes a sender hook for later SMTP parity. |
| GET | `/api/v1/admin/audit-logs` | python | Admin audit log read moved to Python. Requires `AUDITOR` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves Java filters, details fallback, UTC timestamps, and page envelope. |
| POST | `/api/v1/admin/search/rebuild` | python | Admin search-index rebuild moved to Python. Requires `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, rebuilds `skill_search_document` for ACTIVE skills, and writes `REBUILD_SEARCH_INDEX` audit. |
| GET | `/api/v1/admin/skill-reports` | python | Admin skill report list moved to Python. Requires `SKILL_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves Java status parsing, skill/namespace summary projection, and page envelope. |
| POST | `/api/v1/admin/skill-reports/{reportId}/resolve` | python | Admin skill report resolve moved to Python. Requires `SKILL_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, restricts `RESOLVE_AND_HIDE` to `SUPER_ADMIN`, preserves report state mutation, optional skill hide/archive side effects, audit logs, legacy report notification, and upstream `REPORT_RESOLVED` bell notification to the reporter. |
| POST | `/api/v1/admin/skill-reports/{reportId}/dismiss` | python | Admin skill report dismiss moved to Python. Requires `SKILL_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves pending-only transition, trimmed handle comment, audit log, legacy report notification, and upstream `REPORT_RESOLVED` bell notification to the reporter. |
| GET | `/api/v1/admin/profile-reviews` | python | Admin profile review list moved to Python. Requires `USER_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves Java status parsing, sort behavior, JSON snapshot fallback, reviewer projection, and page envelope. |
| POST | `/api/v1/admin/profile-reviews/{id}/approve` | python | Admin profile review approve moved to Python. Requires `USER_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves pending-only transition, display-name application, and audit log. |
| POST | `/api/v1/admin/profile-reviews/{id}/reject` | python | Admin profile review reject moved to Python. Requires `USER_ADMIN` or `SUPER_ADMIN`, rejects bearer API-token principals as Java-compatible unsupported admin-route access, preserves pending-only transition, review comment, and audit log detail JSON. |
...
| DELETE | `/api/v1/skills/id/{skillId}` | python | Whole-skill hard delete by id moved to Python. Requires `SUPER_ADMIN`; bearer `api_token` principals must include `skill:delete`; mock-user development auth keeps precedence. Deletes search document, related review/promotion/social/report/version/file/security rows, records `DELETE_SKILL_HARD`, and deletes local storage with compensation on failure. |
| DELETE | `/api/v1/skills/{namespace}/{slug}` | python | Whole-skill hard delete by namespace/slug moved to Python. Requires `SUPER_ADMIN`; bearer `api_token` principals must include `skill:delete`; mock-user development auth keeps precedence. Normalizes leading `@`, and preserves Java idempotent `deleted=false` for missing or ambiguous slug targets. |
| DELETE | `/api/web/skills/id/{skillId}` | python | Frontend whole-skill hard delete by id moved to Python. Allows `SUPER_ADMIN` or skill owner and shares the same side-effect contract as the v1 id route. Bearer `api_token` principals are rejected as Java-compatible unsupported `403`; mock-user development auth keeps precedence. |
| DELETE | `/api/web/skills/{namespace}/{slug}` | python | Frontend whole-skill hard delete by namespace/slug moved to Python. Allows `SUPER_ADMIN` or skill owner and preserves Java idempotent `deleted=false` for missing or ambiguous slug targets. Bearer `api_token` principals are rejected as Java-compatible unsupported `403`; mock-user development auth keeps precedence. |
| POST | `/api/v1/account/merge/initiate` | python | Account merge initiation moved to Python. Requires current mock user, resolves secondary local username or provider subject, rejects pending/local-credential conflicts, stores BCrypt-compatible verification token hashes, and returns the raw verification token for the dev merge workflow. |
| POST | `/api/v1/account/merge/verify` | python | Account merge verification moved to Python. Requires current mock user, enforces pending owned requests, token expiration, and BCrypt token match before moving the request to `VERIFIED`. |
| POST | `/api/v1/account/merge/confirm` | python | Account merge confirmation moved to Python. Requires current mock user and a verified owned request, atomically moves secondary bindings/tokens/roles/memberships/local credential, fills primary email when blank, marks secondary `MERGED`, and completes the merge request. |
| POST | `/api/v1/auth/device/code` | python | Device authorization code request moved to Python. Generates Java-compatible device/user code payloads, uses Redis keys `device:code:{deviceCode}` and `device:usercode:{userCode}`, and preserves 900-second expiry plus 5-second polling interval. |
| POST | `/api/v1/device/authorize` | python | Device authorization confirmation moved to Python. Requires current mock user, preserves pending/authorized/used user-code transitions, same-user idempotency, different-user rejection, and writes `DEVICE_AUTHORIZE` audit detail with `userCode`. |
