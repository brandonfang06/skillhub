...
| DELETE | `/api/v1/skills/{namespace}/{slug}` | python | Whole-skill hard delete by namespace/slug moved to Python. Requires `SUPER_ADMIN`; bearer `api_token` principals must include `skill:delete`; mock-user development auth keeps precedence. Normalizes leading `@`, preserves Java idempotent `deleted=false` for missing or ambiguous slug targets, and keeps ClawHub one-segment delete Java-owned. |
