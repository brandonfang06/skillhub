...
describe('sync command', () => {
  ...
  test('pull force replaces a changed managed skill with the verified remote snapshot', async () => {
    const env = await createTempHome()
    const skillsDir = join(env.cwd, 'team-skills')
    const remoteBody = '---\nname: demo\ndescription: Demo\nversion: 1.0.0\n---\n'
    ...
    try {
      ...
      expect(JSON.parse(forced.stdout).actions).toContainEqual({ slug: 'demo', action: 'updated' })
  ...
  test('prune preserves a changed orphan unless force is explicit', async () => {
    const env = await createTempHome()
    const skillsDir = join(env.cwd, 'team-skills')
    const fixture = makeSkill('---\nname: demo\ndescription: Demo\nversion: 1.0.0\n---\n')
    const skills: FakeSkill[] = [{ namespace: 'team-a', slug: 'demo', ...fixture }]
    ...
    try {
      ...
      skills.splice(0, skills.length)
      await writeFile(join(skillsDir, 'demo', 'SKILL.md'), '# changed orphan\n')
      ...
      expect(await readFile(join(skillsDir, 'demo', 'SKILL.md'), 'utf8')).toBe('# changed orphan\n')

      const forced = await runCli([
        'sync', 'pull', '--namespace', 'team-a', '--dir', skillsDir, '--prune', '--force',
        '--registry', registry.url, '--token', 'token', '--json'
      ], { HOME: env.home }, { cwd: env.cwd })
      expect(forced.exitCode).toBe(0)
      expect(await Bun.file(join(skillsDir, 'demo')).exists()).toBe(false)
